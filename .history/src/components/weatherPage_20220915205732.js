      ...
      <div className="flex h-[28.5rem] weatherContainer">
        {weekDays.map((day, index) => {
          ...
          if (index === 0 && index !== 7) {
            console.log(index)
            return (
              <div key={index} className="absolute flex flex-col items-center px-2 py-1 mx-auto left-[33%] h-[12.5rem] top-[3.5rem] place-content-center w-28 today1">
                <p className="relative single1 top-2">{dayName}</p>
                <p className="relative text-sm xs:text-lg date1 top-2">
                  {month}/{dayDate}/{year}
                </p>
