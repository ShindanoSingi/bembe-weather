...
const WeatherPage = ({ city, state, country, weatherData }) => {
  ...
  return weatherData.length < 1 ? (
    <div >
      ...
      <div className="px-2 w-[98vw] mt-4 md:p-6 message mx-[1vw]">
        <strong className="md:text-2xl">For US City: </strong><span  className="md:text-2xl">Enter the City, or City and State.</span><br></br>
        <strong  className="md:text-2xl">For Other Country's City: </strong><span  className="md:text-2xl">Enter City, or City and Country's Code.</span>
      </div>
      <br></br>
      <br />
      <br />
    ...
    <div className="bodyContainer">
      {/* Display the city */}
      <div className="fixed top-0 w-screen text-xl text-center cityStateCountry text-blue-50">
        {" "}
        {city}, {state}, {country}{" "}
      </div>
      ...
      <div className="flex h-[30.5rem] xs:h-[20.5rem] weatherContainer">
        {weekDays.map((day, index) => {
          ...
          if (index === 0 && index !== 7) {
            console.log(index)
            return (
              <div key={index} className="fixed flex flex-col items-center px-2 py-1 left-[33%] h-[13rem] xs:left-[10%] xs:flex-row xs:h-[4.5rem] xs:justify-baseline top-[3rem] place-content-center xs:w-[80vw] today1">
                <div className="flex flex-col items-center xs:flex xs:items-center xs:flex-col">
                  <p className="relative text-lg single1 ">{dayName}</p>
                <p className="relative text-sm date1 ">
                  {month}/{dayDate}/{year}
                </p>
                </div>
            ...
            if (index === 1) {
              return (
                <div key={index} className="flex justify-around xs:relative ml-1 flex-col items-center py-2 h-[13rem] xs:h-[10rem] xs:top-[10rem] top-[17rem]  today2 w-[24%]  relative mx-auto">
                  <p className="text-lg singleDayH1">{dayName}</p>
              ...
              return (
                <div key={index} className="flex xs:h-[10rem] justify-around flex-col items-center h-[13rem] day py-1 xs:top-[10rem] top-[17rem] today3 relative mx-auto w-[24%]">
                  <p className="text-lg singleDayH1">{dayName}</p>
                  <p className="text-sm date">
                    {month}/{dayDate}/{year}
                  </p>
              ...
              return (
                <div key={index} className="flex xs:h-[10rem] flex-col items-center h-[13rem] day justify-around py-2 xs:top-[10rem] top-[17rem] today4 relative  mx-auto w-[24%]">
                  <p className="text-lg singleDayH1">{dayName}</p>
                  <p className="text-sm date">
                    {month}/{dayDate}/{year}
                  </p>
              ...
              return (
                <div key={index} className="flex justify-around xs:h-[10rem] flex-col items-center day py-2 h-[13rem] xs:top-[10rem] top-[17rem] today5 w-[24%] mr-1 relative">
                  <p className="text-lg singleDayH1">{dayName}</p>
                  <p className="text-sm date">
                    {month}/{dayDate}/{year}
                  </p>
